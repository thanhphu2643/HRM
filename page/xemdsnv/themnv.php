   ...
   $trans = array(
    ...
    $name = strtr($name, $trans);
        try {
            ...
            $sql = "INSERT INTO nguoidung (HoVaTen, NgaySinh, GioiTinh, Sdt, Email, IMG, Stk, MaCV,DiaChi,HeSoLuong,BHXH)
            VALUES ('$hoten', '$ngaysinh', '$gioitinh', '$sdt', '$email', '$newname', '$stk', 3,'$diachi','$HeSoLuong','$BHXH')";
             $sql .= "; INSERT INTO taikhoan (MaND, TenDangNhap, MatKhau) VALUES (LAST_INSERT_ID(), CONCAT('NV','$name', DATE_FORMAT(NOW(), '%d%H%i%s')), MD5('123'))";
            $result = $dbh->exec($sql);
